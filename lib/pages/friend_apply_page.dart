...
class FriendApplyPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GetBuilder<ApplyFriendListController>(
          init: _applyFriendListController,
          builder: (_){
            return ListView.builder(itemBuilder: (context, index){
              final UserModel userModel = _applyFriendListController.applyUserList[index];
              return Container(
                margin: EdgeInsets.only(top: 10), padding: EdgeInsets.only(left: 5, right: 10),
                      ...
                      child: Row(
                        ...
                        children: [
                          ElevatedButton(
                              ...
                              onPressed: ()=> _applyFriendListController.agreeFriendApply(userModel.id)
                          ),
                          SizedBox(width: 10),
                          ElevatedButton(
                              ...
                              onPressed: ()=> _applyFriendListController.refuseFriendApply(userModel.id)
                          )
                        ],
                      ),
                    ),
